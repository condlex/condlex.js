...
function ltts($text, $filename, $vi = 0, $vc = "en-US") {
  $list = [];
  $text = str_replace('etc.', 'etc_', $text);
  $fld = explode(".", $text);
  foreach ($fld as $tx) {
    ...
    if (strlen($tx) > 200) {
      $pos2 = strpos($tx, " ", 150);
      if ($pos2 !== false) {
        array_push($list, str_replace('etc_', 'etc.', substr($tx, 0, $pos2)));
        array_push($list, str_replace('etc_', 'etc.', substr($tx, $pos2 + 1)));
      }
    } else {
      array_push($list, str_replace('etc_', 'etc.', $tx));
    }
  }
  if (count($list) == 1) {
    print_r($list);
